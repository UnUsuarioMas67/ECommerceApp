    ...
    IHttpClientFactory httpClientFactory)
{
    ...
    private async Task<HttpResponseMessage> SendAsyncInner(ApiRequestOptions options)
    {
        ...
        if (options is ApiRequestOptionsJson jsonOptions)
        {
            var json = JsonSerializer.Serialize(jsonOptions.Body, _jsonSerializerOptions);
            request.Content = new StringContent(json, Encoding.UTF8, "application/json");
        } 
        else if (options is ApiRequestOptionsMultipartForm multipartFormOptions)
        {
            request.Content = multipartFormOptions.Content;
        }
...
public record ApiRequestOptionsJson : ApiRequestOptions
{
    public required object Body { get; init; }
}

public record ApiRequestOptionsMultipartForm : ApiRequestOptions
{
    public required MultipartFormDataContent Content { get; init; }
}